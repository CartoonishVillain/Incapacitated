...
public class FabricIncapacitated implements ModInitializer {
    ...
    private static final SimpleConfig CONFIG = SimpleConfig.of("assets/incapacitated").provider(DefaultConfig::provider).request();
...
//        }));
        ...
        FabricIncapacitated.noMercyDamageSourcesMessageID = new ArrayList<>(List.of(BLEEDOUT.location().getPath(), "outOfWorld", "generic", "genericKill", "outsideBorder"));
    }
    ...
    public static void loadConfig() {
        ...
        try {
            ...
            FabricIncapacitated.configData = gson.fromJson(reader, IncapConfigData.class);
            if (FabricIncapacitated.configData != null) {
                FabricIncapacitated.reviveFoods = getFoodForReviving();
                FabricIncapacitated.adrenalineFoods = getFoodForAdrenaline();
                FabricIncapacitated.healingFoods = getFoodForHealing();
                FabricIncapacitated.instantKillDamageSourcesMessageID = getInstantKills();
                getEffectInstances();
            }
            ...
            try (Writer writer = new FileWriter("config/incapacitated.json")) {
                ...
                FabricIncapacitated.configData = IncapConfigData.buildDefaultConfig();
                FabricIncapacitated.reviveFoods = getFoodForReviving();
                FabricIncapacitated.healingFoods = getFoodForHealing();
                FabricIncapacitated.instantKillDamageSourcesMessageID = getInstantKills();
                getEffectInstances();
    ...
    private static ArrayList<String> getFoodForReviving() {
        final String FoodList = FabricIncapacitated.configData.getFoodReviveList();
    ...
    private static ArrayList<String> getFoodForAdrenaline() {
        try {
            final String FoodList = FabricIncapacitated.configData.getFoodAdrenalineList();
            String[] reviveFoods = FoodList.split(",");
            ArrayList<String> reviveFoodList = new ArrayList<>();
            try {
                for (String string : reviveFoods) {
    ...
    private static ArrayList<String> getFoodForHealing() {
        final String FoodList = FabricIncapacitated.configData.getFoodHealList();
    ...
    private static ArrayList<String> getInstantKills() {
        final String instantKillsString = FabricIncapacitated.configData.getInstantKills();
    ...
    private static void getEffectInstances() {
        FabricIncapacitated.effectInstances.clear();
        for (IncapEffectData effectData : FabricIncapacitated.configData.getIncapEffectData()) {
            ...
            if (effect.isPresent()) {
                ...
                FabricIncapacitated.effectInstances.add(effectInstance);
            }
        }
    }
}
