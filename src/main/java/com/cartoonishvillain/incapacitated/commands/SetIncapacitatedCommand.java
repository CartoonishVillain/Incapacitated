...
public class SetIncapacitatedCommand {
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("assets/incapacitated").then(Commands.literal("setDowned").requires(cs -> cs.hasPermission(2))
                .then(Commands.argument("player", GameProfileArgument.gameProfile()).then(Commands.argument("assets/incapacitated", BoolArgumentType.bool()).executes(context -> {
                    return setIncapped(context.getSource(), GameProfileArgument.getGameProfiles(context, "player"),
                            BoolArgumentType.getBool(context, "assets/incapacitated"));
                })
        ))));
